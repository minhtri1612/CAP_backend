...
## Task
- [ ] Viết `xs-security.json` (PDF Security):
...
- [ ] Mock auth CAP (không có IAS/XSUAA thật):
...
- [ ] Service: `requires: 'authenticated-user'`
- [ ] Instance-level (PDF: `where: 'VendorID = $user.vendor'` / `vendor_ID = $user.VendorID`):
...
- [ ] Test `curl -u alice:…` / `carol` / `bob` / `dave` — verify không leak data cross-vendor; carol tạo Contact OK; alice không tạo được Contact nếu restrict như trên
